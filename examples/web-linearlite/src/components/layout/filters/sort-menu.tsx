...
const shortcutKeysBySortingOption = Object.fromEntries(
  Object.entries(sortingOptions).map(([sortingOption, { shortcut }]) => [sortingOption, [shortcut]]),
  ...
  const handleAction = React.useCallback(
    (key: React.Key) => {
      if (typeof key === 'string') {
        toggleSorting(key as SortingOption)
      }
    },
    [toggleSorting],
  )
  ...
  return (
    <MenuTrigger isOpen={isOpen} onOpenChange={setIsOpen}>
      ...
      <Popover className="w-48 bg-white dark:bg-neutral-800 rounded-lg shadow-md border border-neutral-200 dark:border-neutral-700 text-sm leading-none">
        <Menu className="focus:outline-none" selectionMode="multiple" {...keyboardProps} onAction={handleAction}>
          {type !== 'priority' && (
