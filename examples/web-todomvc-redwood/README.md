...
```bash
bun install
bun run --filter livestore-example-web-todomvc-redwood dev
```

The Redwood dev server chooses its own port; watch the terminal output for the correct URL.

## Known issues
- `rwsdk@1.0.0-beta.12` currently expects React 19.3 canary builds. To keep the monorepo aligned we pin React/React DOM/React Server DOM to 19.1.0 instead, which triggers peer-dependency warnings during `bun install` and may surface runtime incompatibilities.
...
Run the smoke test locally with `bun run --filter livestore-example-web-todomvc-redwood test:e2e`.
