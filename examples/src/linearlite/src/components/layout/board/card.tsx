...
export const Card = memo(({ issue, style }: { issue: Issue; style: CSSProperties }) => {
  ...
  return (
    <div key={issue.id} id={issue.id.toString()} className="px-2 pb-2" style={style}>
      <div
        className="p-2 w-full text-sm bg-white dark:bg-neutral-900 rounded-md shadow-sm dark:shadow-none border border-transparent dark:border-neutral-700/50 cursor-pointer h-full"
