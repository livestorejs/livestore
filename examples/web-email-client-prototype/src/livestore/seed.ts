...
export const seedEmailClientData = (store: Store<typeof schema>) => {
  ...
  for (const label of systemLabels) {
    store.commit(events.labelCreated({
      id: label.id,
      name: label.name,
  ...
  const messages = [
    {
      id: 'msg-1',
      content: 'Hi Bob! I\'ve been working on this email client prototype using LiveStore. It demonstrates event sourcing with multiple aggregates. What do you think?',
    ...
    {
      id: 'msg-2', 
      content: 'That sounds amazing, Alice! I love how LiveStore handles real-time sync between aggregates. Can you show me the cross-aggregate event flow?',
    ...
    {
      id: 'msg-3',
      content: 'Sure! When you apply a label to a thread, the Thread aggregate emits ThreadLabelApplied events. The Label aggregate reacts to these events and updates message counts. It\'s a great example of eventual consistency!',
      sender: 'alice@livestore.dev', 
      senderName: 'Alice Cooper',
    ...
    {
      id: 'msg-4',
      content: 'This is so cool! I can see how this would scale with the 1GB client limit by selectively loading thread event logs. The offline-first approach is perfect for email.',
      sender: 'bob@livestore.dev',
      senderName: 'Bob Smith', 
  ...
  for (const message of messages) {
    if (message.type === 'received') {
      store.commit(events.messageReceived({
        id: message.id,
        threadId,
      ...
      store.commit(events.messageSent({
        id: message.id,
        threadId,
        content: message.content, 
        sender: message.sender,
  ...
  store.commit(events.threadLabelApplied({
    threadId,
    labelId: 'inbox',
    appliedAt: now,
  }))
  ...
  store.commit(events.labelMessageCountUpdated({
    labelId: 'inbox',
    delta: 1,
    updatedAt: now,
  }))
  ...
  for (const messageId of ['msg-1', 'msg-2', 'msg-3']) {
    store.commit(events.messageRead({
      messageId,
      isRead: true,
