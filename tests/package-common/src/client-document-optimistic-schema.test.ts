...
describe('Client Document Optimistic Schema', () => {
  describe('Full Set Operations', () => {
    ...
    it('handles non-object events', () => {
      expect(Schema.decodeSync(optimisticSchema)(null)).toEqual(defaultValue)
      expect(Schema.decodeSync(optimisticSchema)('string')).toEqual(defaultValue)
      expect(Schema.decodeSync(optimisticSchema)(123)).toEqual(defaultValue)
    })
  })
  ...
  describe('Partial Set Operations', () => {
    ...
    it('accepts partial updates', () => {
      ...
      const decoded = Schema.decodeSync(optimisticSchema)(partial)
      expect(decoded).toEqual({ field1: 'updated' })
    })

    it('filters out unknown fields', () => {
      const oldPartial = { field1: 'updated', unknownField: 'data' }
      const decoded = Schema.decodeSync(optimisticSchema)(oldPartial)
      expect(decoded).toEqual({ field1: 'updated' })
    })

    it('returns empty object for incompatible partials', () => {
      const incompatible = { field2: 'not-a-number' }
      const decoded = Schema.decodeSync(optimisticSchema)(incompatible)
      expect(decoded).toEqual({})
    })

    it('handles multiple fields with mixed compatibility', () => {
      const mixed = { field1: 'valid', field2: 42 }
      const decoded = Schema.decodeSync(optimisticSchema)(mixed)
      expect(decoded).toEqual({ field1: 'valid', field2: 42 })
    })

    it('handles non-object values', () => {
      expect(Schema.decodeSync(optimisticSchema)(null)).toEqual({})
      expect(Schema.decodeSync(optimisticSchema)(undefined)).toEqual({})
      expect(Schema.decodeSync(optimisticSchema)([])).toEqual({})
    })
  })
})
