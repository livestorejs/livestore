...
Vitest.describe('ElectricSQL specific tests', { timeout: 60000 }, () => {
  let runtime: ManagedRuntime.ManagedRuntime<SyncProviderImpl | HttpClient.HttpClient, never>
  let testId: string

  Vitest.beforeAll(async () => {
    // Only run these tests for the ElectricSQL provider
    if (ElectricProvider.name !== 'ElectricSQL') {
      return
    }
  ...
  Vitest.scopedLive('should throw descriptive error when detecting delete operations', (test) =>
    Effect.gen(function* () {
      // Skip test if this is not the ElectricSQL provider
      if (ElectricProvider.name !== 'ElectricSQL') {
        test.skip()
        return
      }
  ...
  Vitest.scopedLive('should throw descriptive error when detecting update operations', (test) =>
    Effect.gen(function* () {
      // Skip test if this is not the ElectricSQL provider
      if (ElectricProvider.name !== 'ElectricSQL') {
        test.skip('Test only runs for ElectricSQL provider')
        return
      }
