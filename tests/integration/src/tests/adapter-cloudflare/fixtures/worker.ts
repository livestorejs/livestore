...
import { createStoreDoPromise, makeAdapter, restoreStoreDoSyncTarget } from '@livestore/adapter-cloudflare'
...
export class TestStoreDo extends DurableObjectBase {
  ...
  [restore](params: unknown) {
    return restoreStoreDoSyncTarget(this.ctx, params, {
      onUpdate: (storeId) => this.ensureStore({ storeId, resetPersistence: false }),
    })
  }
