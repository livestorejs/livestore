...
runner, `megarepo` CLI, OTEL dashboards, and
...
Entering the shell builds the Nix environment, puts the base toolchain
(Node.js, Bun, pnpm, `dt`, `mono`, etc.) on `$PATH`, and runs the project
`setup` task (which installs pnpm deps, including package-managed lint and
format tools, and builds TypeScript).
...
`pnpm run <task>` when you only need the command entrypoint. Inside
`devenv shell`, the `dt` CLI remains a thin wrapper around
`devenv tasks run` that adds setup dependencies, status-check caching, and OTEL
instrumentation before delegating to those package scripts. From outside the
shell, use `devenv shell dt <task>` or `devenv tasks run <task>` directly.
Most-used tasks:

- `pnpm run ts:build` / `dt ts:build` — build TypeScript across the monorepo
- `pnpm run ts:check` / `dt ts:check` — run the TypeScript build/check
- `pnpm run lint:full` / `dt lint:full` — run lint + format
- `pnpm run lint:full:fix` / `dt lint:full:fix` — auto-fix lint + format
- `pnpm run test:unit` / `dt test:unit` — run unit tests
- `pnpm run docs:dev` / `dt docs:dev` — start the docs workflow
- `pnpm run examples:test` / `dt examples:test` —
  example workflows
...
Export the following before entering `devenv shell`:
