...
- We use a centralized catalog in `genie/repo.ts` combined with [PNPM catalogs](https://pnpm.io/catalogs)
  to maintain consistent dependency versions across the monorepo.
  - All `package.json` files are generated via Genie from `.genie.ts` source files.
  - Use `mono update-deps` to update dependencies across the monorepo.
...
- Use `mono update-deps` to update dependencies automatically with Expo constraints applied.
- For manual updates, modify the catalog in `genie/repo.ts` and regenerate with `genie`.
