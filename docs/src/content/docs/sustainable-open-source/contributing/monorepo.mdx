...
### Required tooling: Nix + devenv + direnv
...
Playwright browsers, etc.). [direnv](https://direnv.net) is also required to
auto-load the environment when you `cd` into the repo.
...
After installing Nix, install [devenv](https://devenv.sh/getting-started/)
and [direnv](https://direnv.net/docs/installation.html) following their
respective install guides.
...
cd livestore
direnv allow
# direnv triggers devenv's setup task which installs deps, runs genie, and
# builds TypeScript. Subsequent `cd`s into the repo auto-load the environment.
```
...
Once the devenv shell is active (via direnv), the `dt` task runner is on
`$PATH`. Run `dt --list` to see all available tasks. Commonly used ones:
