...
- Run `devenv shell mono docs deploy` to deploy the already-built documentation to the dev Netlify site. On `main`, this updates the dev domain (`https://dev.docs.livestore.dev`); on pull requests and feature branches, it uses a branch alias on the dev site.
