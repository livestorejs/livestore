...
Client Durable Objects implement Cloudflare's `[restore]` method (the `restore` symbol exported from `cloudflare:workers`) and return `restoreStoreDoSyncTarget(this.ctx, params, { onUpdate })` from it (see the client Durable Object example above). For every live pull the sync backend stores a *persistent stub* to this target and re-derives it through `[restore]` on each publish, so a hibernated Durable Object is woken only when there is something to deliver. Both the client and the sync backend Worker need the `allow_irrevocable_stub_storage` compatibility flag.
