...
The sync backend passes the subscription's `storeId` as an optional trailing argument, so implementing this method without it stays valid. A Durable Object can be evicted and reconstructed at any time, so a reverse-RPC may arrive at a DO that no longer holds its store in memory. You choose how to handle that:
