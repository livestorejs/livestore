...
| `Schema.optional(T)` | Nullable column | `T \| null` |
...
- Use `Schema.NullOr()` for fields that can be null; SQLite stores the absent value as `NULL`, so a `Schema.optional()` field is read back as `T | null` too
