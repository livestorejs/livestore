...
Create a [`StoreRegistry`](#new-storeregistryconfig) and provide it via [`<StoreRegistryProvider>`](#storeregistryprovider). Wrap in `<Suspense>` to handle loading states and `<ErrorBoundary>` to handle errors:
...
Use the [`storeOptions()`](#storeoptionsoptions) helper for type-safe, reusable configurations, and then create multiple instances of the same store type by using different `storeId` values:
