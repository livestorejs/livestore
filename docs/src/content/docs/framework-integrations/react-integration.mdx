...
Returns a store instance augmented with [`store.useQuery()`](#storeusequeryqueryable) and [`store.useSyncStatus()`](#storeusesyncstatus) hooks.
...
See the <a href="https://github.com/livestorejs/livestore-contrib/tree/main/examples/web-multi-store">Multi-Store example</a> for a complete working application demonstrating various multi-store patterns.


## Technical Notes

- `@livestore/react` uses `React.useState()` under the hood for `useQuery()` to bind LiveStore's reactivity to React's reactivity. Some libraries use `React.useSyncExternalStore()` for similar purposes but `React.useState()` is more efficient for LiveStore's architecture.
