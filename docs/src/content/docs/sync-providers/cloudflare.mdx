...
- `durableObjectState` - The client Durable Object's `ctx` (`DurableObjectState`); mints the live-pull persistent stub via `ctx.restore` and scopes live-pull routing to this instance so it resets on reconstruction

**Features:**
- Direct RPC communication
- Real-time live pull via Cloudflare persistent stubs (requires the `allow_irrevocable_stub_storage` compatibility flag throughout the restore chain)
- Hibernation support

### `restoreStoreDoSyncTarget(ctx, params, options?)`

From `@livestore/adapter-cloudflare`. Return it from the client DO's `[restore]` method: the sync backend re-derives its live-update target through `[restore]` on every publish. Pass the client DO's `ctx` (`DurableObjectState`) so the update is routed to that instance's live pull. `options.onUpdate(storeId)` runs before routing: a reconstructed (store-less) DO can re-boot its store from it and catch up rather than drop the update — see [the Durable Object adapter's recovery options](/platform-adapters/cloudflare-durable-object-adapter).
...
compatibility_flags = [
  ...
  "allow_irrevocable_stub_storage", # Experimental; required only for DO-RPC live pull
...
The two persistent-stub flags are needed only when this Worker serves DO-RPC live pull. `rpc_params_dup_stubs` is already implied by compatibility dates on or after `2026-01-20`.

:::caution
Cloudflare marks persistent-stub storage as experimental, temporary, and inherently insecure across trust boundaries because the target cannot audit or revoke stored capabilities. Every Worker that stores, targets, or participates in restoring the stub must opt in. Before disabling `allow_irrevocable_stub_storage` or adopting Cloudflare's future replacement, delete `rpc-sub:*` subscription rows while the flag is still enabled; existing stored stubs stop working when any Worker in the restore chain disables it.
:::
