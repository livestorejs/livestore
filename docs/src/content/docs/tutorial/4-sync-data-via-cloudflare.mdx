...
Now, create a new `sync` directory called and a new file for the sync backend:
...
- `migrations`: Whenever you introduce a new Durable Object that supports [SQLite storage](https://developers.cloudflare.com/durable-objects/best-practices/access-durable-objects-storage/#create-sqlite-backed-durable-object-class) (or update an existing one), you need to add a migration referencing the new/updates Durable Object class. 
...
Here's GIF demonstrating how a regular Chrome tab, an incognitoe Chrome tab and a Safari tab are staying in sync automatically:

![](../../../assets/tutorial/section-4/1-livestore-cf-sync.gif)
