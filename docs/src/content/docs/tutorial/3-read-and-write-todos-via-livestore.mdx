...
Now on to the fun part! In this section, you'll set up LiveStore so that the todos that you're creating will persist and survive page refreshs and dev server reloads.
...
3. Wrap your root component (in `main.tsx` in your current case) the LiveStore [provider](https://dev.docs.livestore.dev/reference/framework-integrations/react-integration/#livestoreprovider) (which will receive both the web worker and adapter from above as arguments).
4. Update your Vite Config to work with LiveStore.

Notice that there are _two_ distinct "types" of local SQLite instances in this architecture:

- _in-memory_ and sandboxed to each browser session (used via the `@livestore/wa-sqlite` package)
- _persisted_ via OPFS; the persistence is done by the LiveStore web worker (see below)
...
This is boilerplate code that you'll almost necver need to touch. (In this tutorial, it'll only be edited once when we introduce syncing data to Cloudflare.)

:::note[Special syntax for importing Worker files in Vite]
When importing this file, make sure to add the `?worker` extension to the import path to ensure that Vite treats it as a worker file.
:::
...
-  The **LiveStore shared web worker** (`LiveStoreSharedWorker`); you just imported it from `@livestore/adapter-web/shared-worker`, also a _browser_ web worker.
...
Now, your app is set up to starting reading and writing data in a local SQLite database via LiveStore. If you run the app via `pnpm dev`, you'll briefly see a loading screen (implemented via the `renderLoading` prop on the `LiveStoreProvider`) before the todo list UI from the previous steps will appear again:

![](../../../assets/tutorial/section-3/0-livestore-loading.gif)
...
The current version of the app still stores the todos _in-memory_ via the state of your `App` component. However, with the basic LiveStore setup in place, you can now move to persisting your todos inside SQLite via LiveStore.

In order to read todos, you need to define a LiveStore query. LiveStore queries are _reactive_, meaning they will automatically update your UI components when the underlying data changes.

Here's you can define and use a query to fetch all todos from the local SQLite database using LiveStore inside the `App` component (you don't need to update your code yet, we'll provide the full snippet at the end of this section):

```diff title="App.tsx" lang="ts"
function App() {
...
This was only half of the job though: Right now your code would throw a type error because it still uses `setTodos` to update the local state. You need to update this to use an the `todoCreated` and `todoDeleted` events you defined in `src/livestore/schema.ts`:
...
function App() {
  ...
  // Commit an event to the `store` instead updating local React state
  const addTodo = () => {
+    store.commit(
...
1. The `App` component registers a "live query" with the store
...
1. The query fires again because the state of the DB changed; the returned `todos` array now contains the new todo
...
![](../../../assets/tutorial/section-3/1-livestore-persistence.gif)
...
- All regular windows share the same browser context
- SharedWorker is shared across all these tabs/windows
- OPFS storage is shared across the origin
- ✅ Real-time sync works because they're all using the same SharedWorker and storage

**Incognito windows**

- Incognito mode creates a **separate session** from regular browsing
- However, **multiple incognito windows opened in the same incognito session** still share:
- The same SharedWorker instance
- The same OPFS storage (within that incognito session)
- Each incognito **session** gets isolated storage, but windows within that session are NOT isolated from each other

Think of it this way:

- Regular windows = Session A (all windows share the same data)
- Incognito windows = Session B (all incognito windows share the same data)
...
Here's a little GIF demonstrating the current state of the live updates via browser isolation:

![](../../../assets/tutorial/section-3/3-livestore-sync-local-480.gif)

To recap, here's a visualization of the data flow in the app at this point:

![](../../../assets/tutorial/section-3/2-livestore-data-flow.png)
...
Unfortunately, that's not how OPFS works. While the SQLite files do exist _somewhere_ on your file system, it's hidden in the browser's internals and not easily accessible.
...
LiveStore can only exist thanks to its generous sponsors, please consider becoming one of them if it is of value to you. ❤️
...
![](../../../assets/tutorial/section-3/4-livestore-devtools.png)
