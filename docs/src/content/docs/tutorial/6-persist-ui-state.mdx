...
## Update the LiveStore schema with client-only UI state

In your `schema.ts` file, add another table definition to the `tables` object for persisted client-only UI state:
...
- A name for this UI state table.
- The columns for persisted UI state; in your case:
...
Define an explicit client-only event for this state:
...
+      input: Schema.String.pipe(Schema.optional),
...
Then add a materializer that upserts the UI state row.
...
Replace the `useState()` usage for `input` and `filter` with a LiveStore query and explicit setter events:
...
+    .select('input', 'filter')
+    .where({ id: 'default' })
+    .first({ behaviour: 'fallback', fallback: () => ({ input: '', filter: 'All' }) }),
+)
...
The client-only event persists the UI state inside LiveStore while keeping writes explicit.
...
Here, `uiState$` is the query for persisted UI state, ensuring that the todos query automatically reacts whenever that state changes.
