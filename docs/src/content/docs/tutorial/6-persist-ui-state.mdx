...
As we saw in the beginning of the tutorial, the state of an application is typically divived into two categories:

- App state: State that represents the _application data_ a user needs to achieve their goals with the app; in your current case, that's the list of todos. App state is typically lives in the Cloud somewhere (in traditional apps, the Cloud is the source of truth for it; in local-first apps, the data is stored locally and backed up in the Cloud).
- UI state: UI state that is only relevant for a particular browser session.

Many websites have the problem of "losing UI state" on browser refreshes. This can be incredibly frustrating for users, especially when they've already invested a lot of time getting to a certain point in an app (e.g. filling out a form). The site reloads for some reason and they have to start over! With LiveStore, this problem is easily solved!

LiveStore also allows you to persist _UI state_ (e.g. form inputs, active tabs, custom UI elements, and pretty much anything you'd otherwise manage via `React.useState`). This means users can always pick up exactly where they left off.
...
![](../../../assets/tutorial/section-6/0-tabbed-filter-element.png)
...
Instead of using `useState` for managing the UI state, you're now comitting it into your local SQLite DB via LiveStore as well.
...
You have now recreated the same functionality from before and are able to switch the tabs in the UI element—with one important difference: If you refresh the browser, the UI state will remain the same as before. Your UI state now is persisted and survives page refreshs.

![](../../../assets/tutorial/section-6/1-persist-ui-state.gif)

:::note[Tables vs Client documents]

Unlike the regular tables (of type `State.SQLite.table`), client documents (of type `State.SQLite.clientDocument`) are not synced across browser sessions and devices. 
...
As before, you can deplot the app with the deploy script:

```bash
pnpm run deploy
```

The filters now will be applied and upate the list of todos when changed:

![](../../../assets/tutorial/section-6/2-final-app.gif)