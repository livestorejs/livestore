...
const todos$ = queryDb(() =>
  tables.todos.where({
    completed: filter === 'Completed' ? true : filter === 'Active' ? false : undefined, // if `undefined` is passed to `where`, no filtering happens
  }),
)
const todos = store.useQuery(todos$)
```
...
function App() {
  ...
  const addTodo = () => {
    if (input.trim()) {
      store.commit(events.todoCreated({ id: Date.now(), text: input }))
      updatedInput('')
    }
  }
          ...
          {(['All', 'Active', 'Completed'] as const).map((tab) => (
            <button
              ...
              className={`px-5 py-2.5 text-sm bg-transparent border-0 -mb-px cursor-pointer outline-none transition-colors ${
                filter === tab ? 'text-blue-500 font-semibold' : 'text-gray-600 hover:text-gray-800'
              }`}
            >
              {tab}
            </button>
