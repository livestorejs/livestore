...
export const useRcResource = <T>(
  ...
  const resource = createMemo(
    on(key, (key) => {
      ...
      // SOLID  - why are we manually diffing here?
      //          since we are deliberately listening on `key` do we still need this?

      // Check if the key has changed (or is undefined)
      if (keyRef !== undefined && keyRef !== key) {
        // If the key has changed, decrement the reference on the previous key
        const previousKey = keyRef
  ...
  createEffect(
    on(key, (key) => {
      onCleanup(() => {
        if (didDisposeInMemo) {
          // console.debug('unmount', keyRef, 'skip')
          didDisposeInMemo = false
          return
        }

        // console.debug('unmount', keyRef)
        const cachedItem = cache.get(key)
  ...
  // SOLID  - this seems a bit sketchy?
  keyRef = key()
