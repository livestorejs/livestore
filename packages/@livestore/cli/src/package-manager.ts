 ...
 * Yarn and pnpm are explicitly not supported due to compatibility issues.
 */
export type PackageManager = 'npm' | 'bun'

/**
 * Result of package manager detection.
 * Returns 'unsupported' for yarn and pnpm to allow the CLI to show a warning message.
 */
export type DetectPackageManagerResult =
  | { _tag: 'supported'; pm: PackageManager }
  | { _tag: 'unsupported'; pm: 'yarn' | 'pnpm' }
 ...
 *
 * - Returns 'unsupported' for yarn or pnpm so the CLI can show a recommendation to use bun instead
...
export const detectPackageManager = (
  ...
  if (userAgent.startsWith('pnpm/')) return { _tag: 'unsupported', pm: 'pnpm' }
