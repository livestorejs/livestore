// https://github.com/Effect-TS/effect/pull/5372 TODO remove this once the PR is merged
/**
 * @experimental
 * @since 3.18.0
 */
...
const ProtoGraph = {
  ...
  pipe() {
    // biome-ignore lint/complexity/noArguments: TODO
...
export const dijkstra = <N, E, T extends Kind = 'directed'>(
  ...
  const priorityQueue: Array<{ node: NodeIndex; distance: number }> = [{ node: source, distance: 0 }]
...
export const floydWarshall = <N, E, T extends Kind = 'directed'>(
  ...
  for (const i of allNodes) {
    ...
    for (const j of allNodes) {
      dist.get(i)!.set(j, i === j ? 0 : Number.POSITIVE_INFINITY)
  ...
  for (const k of allNodes) {
    for (const i of allNodes) {
      for (const j of allNodes) {
        ...
        if (distIK !== Number.POSITIVE_INFINITY && distKJ !== Number.POSITIVE_INFINITY && distIK + distKJ < distIJ) {
  ...
  for (const i of allNodes) {
    ...
    for (const j of allNodes) {
      ...
      } else if (dist.get(i)!.get(j)! === Number.POSITIVE_INFINITY) {
...
export const astar = <N, E, T extends Kind = 'directed'>(
  ...
  // Iterate directly over node keys
  for (const node of graph.nodes.keys()) {
    gScore.set(node, node === source ? 0 : Number.POSITIVE_INFINITY)
    fScore.set(node, Number.POSITIVE_INFINITY)
    previous.set(node, null)
  }
  ...
  const openSet: Array<{ node: NodeIndex; fScore: number }> = [{ node: source, fScore: fScore.get(source)! }]
  ...
  if (targetGScore === Number.POSITIVE_INFINITY) {
    return Option.none() // No path exists
  }
...
export const bellmanFord = <N, E, T extends Kind = 'directed'>(
  ...
  for (let i = 0; i < nodeCount - 1; i++) {
    ...
    for (const edge of edges) {
      ...
      // Relaxation step
      if (sourceDistance !== Number.POSITIVE_INFINITY && sourceDistance + edge.weight < targetDistance) {
