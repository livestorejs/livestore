...
export const offerAll = <A>(self: BucketQueue<A>, elements: ReadonlyArray<A>) =>
  Effect.tx(TxRef.update(self, (bucket) => Array.appendAll(bucket, elements)))

export const replace = <A>(self: BucketQueue<A>, elements: ReadonlyArray<A>) =>
  Effect.tx(TxRef.set(self, elements as A[]))

export const clear = <A>(self: BucketQueue<A>) => Effect.tx(TxRef.set(self, []))
...
export const peekAll = <A>(bucket: BucketQueue<A>) => Effect.tx(TxRef.get(bucket))
...
export const size = <A>(bucket: BucketQueue<A>) => peekAll(bucket).pipe(Effect.map((_) => _.length))
