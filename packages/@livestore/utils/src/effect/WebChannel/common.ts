import type { Deferred, Exit } from 'effect'
...
export interface WebChannel<MsgListen, MsgSend, E = never> {
  ...
  listen: Stream.Stream<Exit.Exit<MsgListen, Schema.SchemaError>, E>
...
export const listenToDebugPing =
  (channelName: string) =>
  <MsgListen>(
    stream: Stream.Stream<Exit.Exit<MsgListen, Schema.SchemaError>>,
  ): Stream.Stream<Exit.Exit<MsgListen, Schema.SchemaError>> =>
    stream.pipe(
      Stream.filterEffect(
        Effect.fn(function* (msg) {
          if (msg._tag === 'Success' && Schema.is(DebugPingMessage)(msg.value) === true) {
            yield* Effect.logDebug(`WebChannel:ping [${channelName}] ${msg.value.message}`, msg.value.payload)
            return false
          }
          return true
        }),
