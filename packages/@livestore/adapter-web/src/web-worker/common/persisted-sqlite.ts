...
export class PersistedSqliteError extends Schema.TaggedErrorClass<PersistedSqliteError>(
...
const pruneArchiveDirectory = Effect.fn('@livestore/adapter-web:pruneArchiveDirectory')(function* ({
  ...
  const filesToDelete = pipe(
    filesWithMetadata,
    // oxlint-disable-next-line unicorn/no-array-sort -- EffectArray has no toSorted helper in the current Effect version; sort returns a sorted copy.
