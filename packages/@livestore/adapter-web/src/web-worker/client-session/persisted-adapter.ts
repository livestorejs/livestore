...
export const makePersistedAdapter =
  ...
  (adapterArgs) =>
    Effect.gen(function* () {
      ...
      const sharedWorkerLayer = EffectRpcClient.layerProtocolWorker({ size: 1, concurrency: 100 }).pipe(
        Layer.provide(BrowserWorker.layer(() => sharedWebWorker)),
      )
      const sharedWorkerFiber = yield* Effect.gen(function* () {
        const scope = yield* Effect.scope
        const protocolContext = yield* Layer.buildWithScope(sharedWorkerLayer, scope)
        return yield* EffectRpcClient.make(WorkerSchema.SharedWorkerRpcs).pipe(Effect.provide(protocolContext))
      }).pipe(
        Effect.tapCauseLogPretty,
        Effect.orDie,
      ...
      const runInWorkerStream = <A, E>(tag: string, stream: Stream.Stream<A, E, any>): Stream.Stream<A, never, never> =>
        Effect.gen(function* () {
          yield* Fiber.join(sharedWorkerFiber)
      ...
      const sharedWorker = yield* Fiber.join(sharedWorkerFiber)
      const bootStatusFiber = yield* runInWorkerStream('BootStatusStream', sharedWorker.BootStatusStream(undefined)).pipe(
      ...
      const initialResult =
        dataFromFile === undefined
          ? yield* runInWorker('GetRecreateSnapshot', sharedWorker.GetRecreateSnapshot(undefined)).pipe(
      ...
      const leaderThread: ClientSession['leaderThread'] = {
        export: runInWorker('Export', sharedWorker.Export(undefined)).pipe(
        ...
        events: {
          pull: ({ cursor }) =>
            runInWorkerStream('PullStream', sharedWorker.PullStream({ cursor })).pipe(Stream.orDie),
          push: (batch) =>
        ...
        getEventlogData: runInWorker('ExportEventlog', sharedWorker.ExportEventlog(undefined)).pipe(
          Effect.timeoutOrDie(10_000),
          Effect.withSpan('@livestore/adapter-web:client-session:getEventlogData'),
        ),

        syncState: Subscribable.make({
          get: runInWorker('GetLeaderSyncState', sharedWorker.GetLeaderSyncState(undefined)).pipe(
            Effect.withSpan('@livestore/adapter-web:client-session:getLeaderSyncState'),
          ),
          changes: runInWorkerStream('SyncStateStream', sharedWorker.SyncStateStream(undefined)).pipe(Stream.orDie),
        }),
        ...
        networkStatus: Subscribable.make({
          get: runInWorker('GetNetworkStatus', sharedWorker.GetNetworkStatus(undefined)),
          changes: runInWorkerStream('NetworkStatusStream', sharedWorker.NetworkStatusStream(undefined)),
        }),
      }
