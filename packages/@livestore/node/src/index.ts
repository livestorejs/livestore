...
export const makeNodeAdapter = ({
  ...
  Effect.gen(function* () {
    ...
    return (): Adapter => {
      return (({ schema, storeId, devtoolsEnabled, shutdown, connectDevtoolsToStore }) =>
        Effect.gen(function* () {
          ...
          const leaderThreadFiber = yield* Worker.makePoolSerialized<
            typeof WorkerSchema.LeaderWorkerInner.Request.Type
          >({
            size: 1,
            concurrency: 100,
            initialMessage: () =>
          ...
          yield* Effect.gen(function* () {
            const items = yield* Queue.takeBetween(executionBacklogQueue, 1, 100).pipe(
          ...
          const initialMutationEventId = yield* runInWorker(
            new WorkerSchema.LeaderWorkerInner.GetCurrentMutationEventId(),
          )
        ...
        }).pipe(Effect.withSpan('@livestore/node:adapter'))) satisfies Adapter
    }
  })
