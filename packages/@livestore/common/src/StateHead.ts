 ...
 * `__livestore_session_changeset` as an implicit state-head marker. The fallback
 * stays inside this service so new call sites do not depend on the legacy table.
 */
export interface StateHeadService {
  ...
  /** Returns the current state database head, defaulting to ROOT for an empty state database. */
  get: Effect.Effect<EventSequenceNumber.Client.Composite>
}

export class StateHead extends Context.Service<StateHead, StateHeadService>()('@livestore/common/StateHead') {}
...
export const make = ({ dbState }: { dbState: SqliteDb }) => {
  const getStateHeadOption = Effect.sync(() => {
    ...
    const row = dbState.select<SystemTables.StateHeadMetaRow>(statement, prepareBindValues(bindValues, statement))[0]

    return Option.fromNullishOr(row).pipe(Option.map(rowToStateHead))
  })
  ...
  // Older stores used the session changeset table as an implicit state-head marker.
...
const ensureStateHeadTable = (dbState: SqliteDb) => {
  ...
  // Legacy persisted web snapshots can be imported before a full migration runs.
  dbState.execute(
