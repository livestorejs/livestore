...
import type { InitialSyncInfo, InitialSyncOptions } from './types.js'
import { LeaderThreadCtx } from './types.js'

export const recreateDb = (options: {
  initialSyncOptions: InitialSyncOptions
}): Effect.Effect<
  { snapshot: Uint8Array; syncInfo: InitialSyncInfo },
...
> =>
  Effect.gen(function* () {
    ...
    if (options.initialSyncOptions._tag === 'Blocking') {
      syncInfo = yield* fetchAndApplyRemoteMutations(tmpSyncDb, false, ({ done, total }) =>
        Queue.offer(bootStatusQueue, { stage: 'syncing', progress: { done, total } }),
      ).pipe(
        Effect.timeout(options.initialSyncOptions.timeout),
