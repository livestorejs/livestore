...
import { LeaderThreadCtx } from './mod.js'
...
export const rebasePushQueue = (newUpstreamEvents: MutationEvent.AnyEncoded[]) =>
  Effect.andThen(LeaderThreadCtx, (ctx) =>
    Effect.gen(function* () {
...
export interface PushQueueLeader {
  ...
  push: (item: PushQueueItemLeader) => Effect.Effect<void, never, LeaderThreadCtx>
}

const makePushQueueLeader = (syncDb: SynchronousDatabase, currentMutationEventIdRef: { current: EventId }) =>
  Effect.gen(function* () {
    const mode = 'leader'

    // TODO rehydrate from db
    const items: PushQueueItemLeader[] = []
