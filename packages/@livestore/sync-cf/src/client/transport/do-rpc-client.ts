...
import { SyncDoRpc, SyncDoRpcPullPayload } from '../../common/do-rpc-schema.ts'
    ...
    Effect.gen(function* () {
      ...
      const ProtocolLive = layerProtocolDurableObject({
        callRpc: (payload, request) => {
          const live = livePullParamsOf(request, serialization.codecFor)
...
export const handleSyncUpdateRpc = (ctx: CfTypes.DurableObjectState, payload: Uint8Array<ArrayBuffer>) =>
  Effect.gen(function* () {
    ...
    const decoded = yield* Schema.decodeEffect(serialization.codecFor(Schema.NonEmptyArray(SyncMessage.PullResponse)))(
...
const livePullParamsOf = (
  request: RpcMessage.RequestEncoded,
  codecFor: RpcSerialization.CodecFor,
): SyncUpdateRestoreParams | undefined => {
  if (request.tag !== 'SyncDoRpc.Pull') return undefined
  const decoded = Schema.decodeUnknownOption(codecFor(SyncDoRpcPullPayload))(request.payload)
  return Option.isSome(decoded) === true && decoded.value.live !== undefined
