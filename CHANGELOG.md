...
- **Declared release credential surfaces + weekly liveness check:** The external credentials the production deploy steps use (Netlify, Cloudflare, Mixedbread) are now declared once in `genie/release-credentials.ts` alongside a read-only probe for each. `release.yml` derives its `env:` blocks from that declaration and a new `health-release-credentials.yml` derives a weekly probe from it, so the two can no longer disagree about which credentials a release needs. A secret that is referenced but never defined ([#1284](https://github.com/livestorejs/livestore/issues/1284)) now arrives empty and fails the weekly probe, instead of staying hidden until a release reaches that deploy step. The check makes only HTTP calls and sets up no toolchain — a release dry-run already proves release code works; it cannot prove a token that expires on wall-clock time is still accepted.
- **Registry-verified, attested stable releases:** Stable publishing now emits SLSA provenance (`--provenance`) under GitHub Actions, matching snapshot and DevTools artifact publishing — previously stable releases were the only artifact shipped unattested. After publishing, every package is verified against what the registry actually serves: version visibility, tarball digest against the locally packed artifact, and that the release's dist-tag resolves to exactly that version. A publish that succeeds while `latest` still points at the previous release now fails the release instead of silently leaving `npm install` on the old version.
