...
- **Declared release credential surfaces + weekly liveness check:** The external credentials the production deploy steps use (Netlify, Cloudflare, Mixedbread) are now declared once in `genie/release-credentials.ts` alongside a read-only probe for each. `release.yml` derives its `env:` blocks from that declaration and a new `health-release-credentials.yml` derives a weekly probe from it, so a secret referenced by a release step but never defined ([#1284](https://github.com/livestorejs/livestore/issues/1284)) is unrepresentable rather than merely detectable. The check makes only HTTP calls and sets up no toolchain — a release dry-run already proves release code works; it cannot prove a token that expires on wall-clock time is still accepted.
