...
# Mixedbread vector store ids — opaque, non-secret identifiers (the API key is
# the actual gate). One per docs surface: each surface searches only the content
# it serves (LS.DOCS.SEARCH-R03), so production reads the store fed by stable
# releases and development reads the store fed by pushes to `main`.
variable "mxbai_vector_store_id_prod" {
  type    = string
  default = "fff51624-7624-44b1-a223-71a6e29f7f02"
}

variable "mxbai_vector_store_id_dev" {
  type    = string
