# Multi-Store Design Proposal (Global Registry + Promise-first)
...
import { getStore } from '@livestore/react'

// ... store definition code from above ...

export function WorkspaceView() {
  const workspaceStore = React.use(getStore(workspaceStoreDef))
...
export function IssueView({ issueId }: { issueId: string }) {
  const issueStore = React.use(getStore(issueStoreDef, { storeId: issueId }))
...
```ts
type GetStore = <TSchema extends LiveStoreSchema>(
...
function MainContent() {
  const appStore = React.use(getStore(appStoreDef)) // Suspends the component until the store is ready
...
function WorkspaceView() {
  const workspaceStore = React.use(getStore(workspaceStoreDef)) // Defaults to storeId = workspaceStoreDef.name
  const workspace = workspaceStore.useQuery(workspaceQuery)
  return <div>Workspace: {workspace.name}</div>
}
...
export const Route = createFileRoute('/workspaces/$workspaceId')({
  loader: async ({ params }) => {
    await getStore(workspaceStoreDef, { storeId: params.workspaceId }, registry)
    return null
  },
...
function WorkspaceRouteComponent() {
  ...
  const workspaceStore = React.use(getStore(workspaceStoreDef, { storeId: workspaceId }))
  const workspace = workspaceStore.useQuery(workspaceQuery(workspaceId))
...
let currentRegistry: StoreRegistry = defaultRegistry
...
function resolveRegistry(registryOverride?: StoreRegistry) {
  return registryOverride ?? currentRegistry
}

// --- Promise-first store accessors ---
export function getStore<TSchema extends LiveStoreSchema>(
  def: StoreDefinition<TSchema>,
  params?: CreateStoreParams,
  registryOverride?: StoreRegistry
): Promise<Store<TSchema>> {
  const registry = resolveRegistry(registryOverride)
  return registry.get(def, params)
}

export function preloadStore<TSchema extends LiveStoreSchema>(
  def: StoreDefinition<TSchema>,
  params?: CreateStoreParams,
  registryOverride?: StoreRegistry
): Promise<Store<TSchema>> {
  return getStore(def, params, registryOverride)
}
```
