> Status: pending dissolution (2026-07-16) — the TODO/bug items are task
> material, not intent; the "Work notes" and "Future work" sections hold
> design evidence (rejected CF transport explorations, 2-phase pull
> rationale, deployment directions) not yet captured in `context/` — see
> `context/02-system/03-sync/03-cf/` and `04-runtime/02-cloudflare/`.
...
- [ ] CF DO adapter (`@livestore/adapter-cloudflare`)
  ...
  - [ ] Ungraceful error handling when DO/client gets into an inconsistent state (e.g. for a unique constraint violation in the materializer)
    ```
    [19:12:55.160 DoClient] UnknownError: { "cause": Error: This should never happen: During boot the backend head (9) should never be greater than the local head (8), "note": undefined, "payload": undefined } UnknownError: { "cause": Error: This should never happen: During boot the backend head (9) should never be greater than the local head (8), "note": undefined, "payload": undefined }
    ```
