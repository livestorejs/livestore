...
const todos = State.SQLite.table({
  ...
  columns: {
    ...
    deleted: State.SQLite.integer({ nullable: true, schema: Schema.DateFromNumber }), // Optional field to mark deletion
  },
})
...
export const schema = makeSchema({
  tables,
  events,
  migrations: { strategy: 'from-eventlog' }, // Define migration strategy
})
...
Use the `defineMutation`, `Schema`, and `sql` modules from `@livestore/livestore` to define these functions. These tools let you create fully typed events for secure and efficient database interactions.
...
const boot = (store: Store) => {
  ...
  if (store.__select(sql`SELECT count(*) as count FROM todos`)[0]!.count === 0) {
    store.commit(events.todoCreated({ id: cuid(), text: '☕ Make coffee' }))
  }
}
```
...
import { todoDeleted } from '@/livestore/events'
...
export default function HomeScreen() {
  // Define a query
  const todosQuery = queryDb(tables.todos)
