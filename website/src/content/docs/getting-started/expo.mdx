...
Create a file called `mutations.ts` inside the `schema` folder. This file contains the mutations your app needs to interact with the database.

Use `Schema` from `effect`, along with `defineMutation` and `sql` from `@livestore/livestore`, to define fully typed, robust mutations. These functions allow you to create safe, consistent database operations.
