...
- **LS.SYS.RT-A01 Cooperative contexts:** All execution contexts of one
  client (sessions, leader) can exchange messages and transferables through
  some platform channel primitive.

## Requirements

### Leader ⇄ client-session topology

- **LS.SYS.RT-R01 Single leader per client:** `refines: LS-R04` — At any
  time exactly one leader per client owns the persisted eventlog, the state
  database, and upstream sync.
- **LS.SYS.RT-R02 Proxy-only leader access:** Client sessions interact with
  the leader exclusively through the leader-thread proxy contract (event
...
- **LS.SYS.RT-R03 Session resume:** A session boots from the leader's initial
  state (leader head cursor, migrations report, storage mode) and resumes
  event streaming from its cursor without event loss or duplication.
...
- **LS.SYS.RT-R05 Adapter realization:** `refines: LS-R07` — An adapter
  instantiates a client session for its platform, providing: a session-local
  SQLite database, a leader-thread proxy, lock status, shutdown handling, and
  devtools connectivity (where supported).
...
- **LS.SYS.RT-R09 Portable SQLite substrate:** Leader and session databases
  run on the same WASM SQLite build exposed through per-platform entrypoints
