...
- Hibernated-client delivery gaps become possible on DO-RPC (issue #1415) —
  the price of not pinning the client DO awake. (Later resolved without
  pinning: the reverse-RPC now carries `storeId` so a reconstructed client
  re-boots and catches up — see
  [0002-reverse-rpc-storeid-recovery.md](./0002-reverse-rpc-storeid-recovery.md).)
