# 0003 — DO-RPC clients unsubscribe on graceful shutdown; subscriptions are never reaped on silence

Status: accepted (recorded 2026-08-09).

## Context

0002 made a DO-RPC subscription deliberately survive client-DO eviction so a
...
The binding constraint from 0002: teardown must never key off mere absence. A
...
- **(b) Server-side reaping on silence/idle/TTL/failed delivery.** Rejected:
  reintroduces the 0002/#1462 regression — it cannot tell a hibernating client
...
- Graceful teardown is handled; the remaining leak is by design — a client DO
  ...
  reaps on silence (0002). Reaping a permanently-departed client is the
