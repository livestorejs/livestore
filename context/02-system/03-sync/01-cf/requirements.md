...
- **LS.SYS.SYNC.CF-R01 Contract realization:** Realizes the full provider
  contract — ordered persistence, live pull streams (`pullLive: true`), push
  validation — with one Durable Object as the ordering authority per store.
  `refines: LS.SYS.SYNC-R06, LS.SYS.SYNC-R01`
...
- **LS.SYS.SYNC.CF-R03 Schema-defined messages:** All transports share the
  versioned message schemas in `src/common/` (`ws-rpc-schema.ts`,
  `http-rpc-schema.ts`, `do-rpc-schema.ts`, `sync-message-types.ts`).
  `refines: LS.SYS-R02`
- **LS.SYS.SYNC.CF-R04 Reference status:** Serves as the conformance baseline
  for the provider contract (`09-verification/`) and the deploy target for
  examples.
