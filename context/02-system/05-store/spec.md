...
- A poisoned canonical event crosses the adapter shutdown channel as a
  structured failure carrying the event identity, deterministic application
  cause, and last valid head. Lifecycle supervision marks the Store failed and
  closes its lifetime scope regardless of `onSyncError: 'ignore'`; recovery is
  a Store restart after the schema, materializer, state, or canonical data has
  been repaired. Application commit sites do not handle this per event
  (LS.SYS.STORE-R13).
